    ...
    void M_Connect(const Node &node)override
    {
        ...
        if (node.id != Node::kEmpty) {
            ...
            if (my_node_.role == Node::SERVER) {//server
                ...
                m_endpoints_[my_node_.id] =
                    std::make_unique<Endpoint>();  // m_endpoints_表示本地启用的组播连接
        ...
        {  //表示本端去连接node这个节点
            ...
            if (node.id != Node::kEmpty) {
                ...
                struct addrinfo *remote_addr;
                CHECK_EQ(
                    getaddrinfo(node.hostname.c_str(), std::to_string(node.port).c_str(), nullptr, &remote_addr), 0);
        ...
        int SendMsg(Message & msg) override
        {
            ...
            if (!IsValidPushpull(msg)) {                         //控制信息不为空，
                ...
                if (!HasRemoteInfo(msg, key, is_push, remote_id)) {  //这个时候如果没有远端的地址信息
                    ...
                    trans->SendRendezvousBegin(
                        msg, msg_buf);  //将这个请求信息发送，但是发送时并没有携带本端的address和rkey
                    return total_len;
                }
            }
    ...
    private:
        ...
        void PrintRecvLog(Message * msg, BufferContext * buffer_ctx, int meta_len)
        {
            ...
            if (!IsValidPushpull(*msg)) {
                ...
                LOG(INFO) << "Recv Push Request: key=" << msg->meta.key << "\t timestamp=" << msg->meta.timestamp
                          << "\t sender=" << msg->meta.sender << "\t tensor_len=" << buffer_ctx->data_len[1]
                          << std::flush;
            } else if (!msg->meta.push && msg->meta.request) {
                // pull request
                    ...
                    switch (wc[i].opcode) {
                        ...
                        case IBV_WC_RECV: {  // Send data operation for a WR that was posted to a Receive
                            ...
                            if (imm ==
                                ...
                                auto addr_tuple =
                                    GetRemoteAndLocalInfo(msg->meta.key, msg->meta.push, msg->meta.recver);
        ...
        void OnAddrResolved(struct rdma_cm_event * event)
        {
            ...
            if (it == listen_id_list.end()) {
                ...
                endpoint->MulticastInit(cq_, pd_, len);  //自己作为client，初始化cq和pd
                ...
                struct addrinfo *remote_addr;
                std::string addr;  //找到第一个未加入的组播地址，然后加入。如果是server，则未加入的组播地址只有一个，即为绑定自己的组播地址
